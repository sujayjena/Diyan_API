...
namespace Diyan.Persistence.Repositories
{
    public class JwtUtilsRepository : IJwtUtilsRepository
    {
        ...
        public (string, DateTime) GenerateJwtToken_Customer(MobileAppLoginSessionData parameters)
        {
            ...
            if (parameters.UserId != null)
            {
                claims.Add(new Claim("UserName", parameters.CustomerName));
            }
            //else if (parameters.CustomerId != null)
            //{
            //    claims.Add(new Claim("Name", parameters.CompanyName));
        ...
        public async Task<MobileAppLoginSessionData?> ValidateJwtToken_Customer(string token)
        {
            byte[] key;
            JwtSecurityTokenHandler tokenHandler;
            JwtSecurityToken jwtToken;
            MobileAppLoginSessionData? response;
            ...
            try
            {
                ...
                response = await _loginRepository.GetCustomerDetailsByToken(token);
